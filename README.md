...
路由模型显示不需要额外配置开关。HUD 会自动对比 Claude Code 当前请求地址和 `~/.claude-code-router/config.json` 中的 CCR 监听地址；只有确认当前会话正在走 CCR 时，才读取当前会话级模型状态文件。新会话尚未发起首次路由请求时，模型组件会显示 `Routing...`；已有 Token 活动后如果仍缺少会话状态文件，才显示 `CCR真实模型未启用：运行 /claude-hud-plus:setup`，避免把 Claude Code 原始请求模型误显示为真实路由模型。
...
◐ Edit: auth.ts | ◐ explore [haiku]: 查找认证代码 | ▸ 修复认证漏洞（2/5）
```
- **第 1 行** — 模型、上下文进度条和上下文数值
- **第 2 行** — 项目路径、额外工作目录和 git 分支（未使用 `/add-dir` 时只显示项目与 Git）
- **第 3 行** — 当前会话累计 Token
- **第 4 行** — 活跃工具、Agent 状态和待办进度

布局由 `config.json` 中的 `rows` 定义，想显示几行、每行包含哪些组件都可以调整。Claude Code 原生的权限模式提示（如 bypass permissions）不属于 HUD 输出，不需要在这里配置。Plus 默认配置会隐藏使用率限制；如果需要显示订阅使用率，请开启 `display.showUsage`。
...
| `rows` | string[][] | `[["model","contextBar","contextValue"],["project","addedDirs","git"],["sessionTokens"],["tools","agents","todos"]]` | HUD 行布局；外层数组定义行，内层数组定义该行显示的组件 |
...
| `display.showUsage` | boolean | false | 显示 Claude 订阅用户的使用率限制（可用时） |
...
| `display.showAgents` | boolean | true | 显示 Agent 活动行 |
| `display.showTodos` | boolean | true | 显示待办进度行 |
...
| `colors.context` | 颜色值 | `#22D3EE` | 上下文进度条和百分比的基础颜色 |
| `colors.usage` | 颜色值 | `brightBlue` | 使用率进度条和低于警告阈值时百分比的颜色 |
| `colors.warning` | 颜色值 | `#F59E0B` | 上下文阈值和使用率警告文本的警告颜色 |
| `colors.usageWarning` | 颜色值 | `brightMagenta` | 使用率进度条和接近阈值时百分比的警告颜色 |
| `colors.critical` | 颜色值 | `#F43F5E` | 达到限制状态和严重阈值的颜色 |
| `colors.model` | 颜色值 | `#38BDF8` | 模型徽章颜色，如 `[Opus]` |
| `colors.project` | 颜色值 | `#FBBF24` | 项目路径的颜色 |
| `colors.git` | 颜色值 | `#C084FC` | Git 包装文本的颜色，如 `git:(` 和 `)` |
| `colors.gitBranch` | 颜色值 | `#22D3EE` | Git 分支和分支状态文本的颜色 |
...
| `colors.contextBands` | color band[] | `[]` | 可选上下文色带，例如 `{ "min": 70, "color": "yellow" }`；设置后会优先于 `context` / `warning` / `critical` 控制上下文进度条颜色 |
| `colors.usageBands` | color band[] | `[]` | 可选使用率色带；设置后会优先于 `usage` / `usageWarning` / `critical` 控制使用率进度条颜色 |

`colors.barFilled` 和 `colors.barEmpty` 只接受单个可见字素。控制字符、不可见格式字符、换行分隔符和非字符会被拒绝。宽字符（emoji、CJK）在不同终端中可能影响对齐。

支持的颜色名称：`dim`、`red`、`green`、`yellow`、`magenta`、`cyan`、`brightBlue`、`brightMagenta`。你也可以使用 256 色数字（`0-255`）或十六进制（`#rrggbb`）。色带使用同样的颜色值格式；无效条目会被过滤，重复 `min` 会保留靠前的条目，并在渲染前按 `min` 从高到低排序。
