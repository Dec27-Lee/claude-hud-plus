...
import { applyContextWindowSizeOverride } from './plus/context-window-override.js';
import { getRouterModelInfo } from './plus/router-model.js';
...
export async function readStdin(
  ...
  return await new Promise<StdinData | null>((resolve) => {
    ...
    const tryParse = (): StdinData | null | undefined => {
      ...
      try {
        const parsed = JSON.parse(trimmed) as StdinData;
        applyContextWindowSizeOverride(parsed);
        return parsed;
      } catch {
        return undefined;
      }
...
export function getModelName(stdin: StdinData): string {
  const routerModel = getRouterModelInfo(stdin)?.model;
  if (routerModel) {
    return routerModel;
  }
