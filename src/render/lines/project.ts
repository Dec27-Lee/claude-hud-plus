...
import { normalizeAddedDirs, sanitize as sanitizeDisplayText, basenameOf, truncateBasename, MAX_RENDERED_ADDED_DIRS, formatMore } from './added-dirs.js';
...
export function renderModelPart(ctx: RenderContext): string | null {
  ...
  const model = formatModelName(
    routerStatus.kind === 'ready' ? routerStatus.info.model : getModelName(ctx.stdin),
    display?.modelFormat,
...
export function renderProjectPart(ctx: RenderContext): string | null {
  const display = ctx.config?.display;
...
export function renderAddedDirsPart(ctx: RenderContext): string | null {
  const display = ctx.config?.display;
...
export function renderGitFilesLine(ctx: RenderContext, terminalWidth: number | null = null): string | null {
  ...
  if (overflow > 0) statParts.push(dim(`+${formatMore(overflow)}`));
